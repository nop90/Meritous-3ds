...
void ShowEnding()
{
	...
	if (streamspr == NULL) {
		streamspr = IMG_Load("romfs:/i/stream.png");
		SDL_SetColorKey(streamspr, SDL_SRCCOLORKEY | SDL_RLEACCEL, 0);
		glitter = IMG_Load("romfs:/i/glitter.png");
	...
	Paint(0, 0, 22, 27, "romfs:/d/cstream.loc");
...
void DrawCredits()
{
	...
	if (fin == NULL) {
		fin = IMG_Load("romfs:/i/fin.png");

		theend[0] = IMG_Load("romfs:/i/theend.png");
		theend[1] = IMG_Load("romfs:/i/true_end.png");
	}
...
void DrawCircuitFlash(int t, int method)
{
	...
	if (circ == NULL) {
		circ = IMG_Load("romfs:/i/circuits_1.png");
	}
